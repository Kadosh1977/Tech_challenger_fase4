...
# --- ADIÇÃO 1: GRÁFICO DE ANÁLISE HISTÓRICA DO PREÇO DE FECHAMENTO ---

# Garante que 'historical_data' tem a coluna 'Data' como datetime
if historical_data is not None:
    st.header("Análise de Preços Históricos (Últimos 100 Dias)")
    
    # Cria o gráfico interativo de linha (Plotly)
    fig_hist = px.line(
        historical_data,
        x='Data',
        y='close',
        title='Preço de Fechamento do IBOVESPA',
        labels={'close': 'Preço de Fechamento'},
        template='plotly_white'
    )
    
    # Personaliza para melhor visualização
    fig_hist.update_traces(line=dict(width=2, color='#0077b6'))
    fig_hist.update_layout(xaxis_title="Data", yaxis_title="Preço (R$)")
